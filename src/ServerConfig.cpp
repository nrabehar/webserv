...
/*   Updated: 2025/08/08 11:41:36 by nrabehar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
const std::vector<size_t> &ServerConfig::getPorts() const { return _ports; }
...
size_t ServerConfig::getClientMaxBodySize() const { return _MaxBodySise; }
...
void ServerConfig::addPort(size_t port) { _ports.push_back(port); }
void ServerConfig::addHost(const std::string &host) { _hosts.push_back(host); }
void ServerConfig::setIsDefault(bool isDefault) { _isDefault = isDefault; }
void ServerConfig::setMaxBodySize(size_t size) { _MaxBodySise = size; }
void ServerConfig::setErrorPage(int code, const std::string &page) { _errorPages[code] = page; }
void ServerConfig::addLocation(const Location &location) { _locations.push_back(location); }
void ServerConfig::setRoot(const std::string &newRoot) { this->_root = newRoot; }

void ServerConfig::fixLocationData()
{
...
void ServerConfig::parseHostPort(const std::string &hostPort)
{
	int port;
	std::string host;
...
void ServerConfig::setProperty(const std::string &key, const std::string &value)
{
	if (key == "listen")
	{
