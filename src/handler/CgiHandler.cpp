...
CgiHandler::CgiStdinHandler::CgiStdinHandler(CgiHandler * cgi, Buffer * in, int fd)
  : _fd(fd), _timeout(30), _last_active(time(NULL)),
  _cgi(cgi), _in(in), _offset(0)
{
...
int CgiHandler::CgiStdinHandler::fd() const
{
  return (_fd);
}
...
void CgiHandler::CgiStdinHandler::setTimeout(int timeout)
{
  _timeout = timeout;
}
...
int CgiHandler::CgiStdoutHandler::fd() const
{
  return (_fd);
}
...
void CgiHandler::CgiStdoutHandler::setTimeout(int timeout)
{
  _timeout = timeout;
}
